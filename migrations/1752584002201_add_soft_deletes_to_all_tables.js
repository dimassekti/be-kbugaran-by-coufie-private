exports.up = (pgm) => {
  ...
  pgm.sql('CREATE UNIQUE INDEX albums_name_year_unique ON albums(name, year) WHERE deleted_at IS NULL');

  // Song title uniqueness per album
  pgm.sql('ALTER TABLE songs DROP CONSTRAINT IF EXISTS songs_title_album_id_key');
  pgm.sql('CREATE UNIQUE INDEX songs_title_album_id_unique ON songs(title, album_id) WHERE deleted_at IS NULL');

  // Playlist name uniqueness per owner
  pgm.sql('ALTER TABLE playlists DROP CONSTRAINT IF EXISTS playlists_name_owner_key');
  pgm.sql('CREATE UNIQUE INDEX playlists_name_owner_unique ON playlists(name, owner) WHERE deleted_at IS NULL');
};
...
exports.down = (pgm) => {
  ...
  pgm.sql('ALTER TABLE albums ADD CONSTRAINT albums_name_year_key UNIQUE (name, year)');

  pgm.sql('DROP INDEX IF EXISTS songs_title_album_id_unique');
  pgm.sql('ALTER TABLE songs ADD CONSTRAINT songs_title_album_id_key UNIQUE (title, album_id)');

  pgm.sql('DROP INDEX IF EXISTS playlists_name_owner_unique');
  pgm.sql('ALTER TABLE playlists ADD CONSTRAINT playlists_name_owner_key UNIQUE (name, owner)');
