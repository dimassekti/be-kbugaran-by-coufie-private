...
class AlbumsService {
  ...
  async addAlbum({ name, year, coverUrl = null }) {
    ...
    const query = {
      text: "INSERT INTO albums (id, name, year, cover_url) VALUES($1, $2, $3, $4) RETURNING id",
  ...
  async getAlbums() {
    const result = await this._pool.query("SELECT id, name, year, cover_url as \"coverUrl\" FROM albums");
    return result.rows;
  }
